...
ansible-playbook playbooks/cashangular.yml -i "dynamic-inventory/$ansible_inventory" --extra-vars "cashapp_angular_version=$file_contents"
    ...
    aws sns publish --topic-arn "arn:aws:sns:us-east-1:148654267025:Cync-Dev-Deployment-Notification" --region us-east-1 --subject "Cash-angular Deployment for $ansible_inventory environment Failed" --message "Deployment trigger Failed due to PING TEST in $ansible_inventory environment for Cash-Angular Version: $file_contents"
    exit 1
fi
if [[  $? != 0 ]]
then
