...
ansible-playbook playbooks/ablangular.yml -i "dynamic-inventory/$ansible_inventory" --extra-vars "abl_angular_version=$file_contents"
else
    echo "ping test got failed No deployment!!"
    aws sns publish --topic-arn "arn:aws:sns:us-east-1:148654267025:Cync-Dev-Deployment-Notification" --region us-east-1 --subject "ABL-Angular Deployment for $ansible_inventory environment Failed" --message "Deployment trigger Failed due to PING TEST in $ansible_inventory environment for ABL-Angular Version:$file_contents"
    exit 1
fi
if [[  $? != 0 ]]
then
