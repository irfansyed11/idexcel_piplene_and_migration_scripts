...
ansible-playbook playbooks/angular.yml -i "dynamic-inventory/$ansible_inventory" --extra-vars "angular_version=$file_contents"
    ...
    aws sns publish --topic-arn "arn:aws:sns:us-east-1:148654267025:Cync-Dev-Deployment-Notification" --region us-east-1 --subject "Cync Angular Deployment to $ansible_inventory environment Failed" --message "Deployment trigger Failed due to PING TEST in $ansible_inventory environment for Angular Version:$file_contents"
    exit 1
fi
if [[  $? != 0 ]]
then
